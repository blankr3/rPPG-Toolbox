...
_C.TRAIN.DATA.AUGMENTATION = CN()
_C.TRAIN.DATA.AUGMENTATION.PHYSICS_SKIN_TONE = False
_C.TRAIN.DATA.AUGMENTATION.PHYSICS_SKIN_P = 0.5
_C.TRAIN.DATA.AUGMENTATION.PHYSICS_SKIN_R = 0.10
...
_C.UNSUPERVISED.DATA.AUGMENTATION = CN()
_C.UNSUPERVISED.DATA.AUGMENTATION.PHYSICS_SKIN_TONE = False
_C.UNSUPERVISED.DATA.AUGMENTATION.PHYSICS_SKIN_P = 0.5
...
def update_config(config, args):
    ...
    if not ext:  # no file extension
        FOLD_STR = (
            "_" + config.TRAIN.DATA.FOLD.FOLD_NAME
            if config.TRAIN.DATA.FOLD.FOLD_NAME
            else ""
        )
        config.TRAIN.DATA.FILE_LIST_PATH = os.path.join(
            config.TRAIN.DATA.FILE_LIST_PATH,
            config.TRAIN.DATA.EXP_DATA_NAME
            + "_"
            + str(config.TRAIN.DATA.BEGIN)
            + "_"
    ...
    if ext == ".csv" and config.TRAIN.DATA.DO_PREPROCESS:
        raise ValueError(
            "User specified TRAIN dataset FILE_LIST_PATH .csv file already exists. \
    ...
    if not config.TEST.USE_LAST_EPOCH and config.VALID.DATA.DATASET is not None:
        ...
        if config.VALID.DATA.EXP_DATA_NAME == "":
            config.VALID.DATA.EXP_DATA_NAME = "_".join(
                [
                    ...
                    "ClipLength{0}".format(
                        str(config.VALID.DATA.PREPROCESS.CHUNK_LENGTH)
                    ),
                    "DataType{0}".format(
                        "_".join(config.VALID.DATA.PREPROCESS.DATA_TYPE)
                    ),
                    "DataAug{0}".format(
        ...
        if not ext:  # no file extension
            FOLD_STR = (
                "_" + config.VALID.DATA.FOLD.FOLD_NAME
                if config.VALID.DATA.FOLD.FOLD_NAME
                else ""
            )
            config.VALID.DATA.FILE_LIST_PATH = os.path.join(
                config.VALID.DATA.FILE_LIST_PATH,
                config.VALID.DATA.EXP_DATA_NAME
                + "_"
                + str(config.VALID.DATA.BEGIN)
                + "_"
        ...
        if ext == ".csv" and config.VALID.DATA.DO_PREPROCESS:
            raise ValueError(
                "User specified VALIDATION dataset FILE_LIST_PATH .csv file already exists. \
    ...
    if not ext:  # no file extension
        FOLD_STR = (
            "_" + config.TEST.DATA.FOLD.FOLD_NAME
            if config.TEST.DATA.FOLD.FOLD_NAME
            else ""
        )
        config.TEST.DATA.FILE_LIST_PATH = os.path.join(
            config.TEST.DATA.FILE_LIST_PATH,
            config.TEST.DATA.EXP_DATA_NAME
            + "_"
            + str(config.TEST.DATA.BEGIN)
            + "_"
    ...
    if ext == ".csv" and config.TEST.DATA.DO_PREPROCESS:
        raise ValueError(
            "User specified TEST dataset FILE_LIST_PATH .csv file already exists. \
    ...
    if any(
        aug != "None"
        for aug in config.TRAIN.DATA.PREPROCESS.DATA_AUG
        + config.VALID.DATA.PREPROCESS.DATA_AUG
        + config.TEST.DATA.PREPROCESS.DATA_AUG
    ):
        ...
        if re.match(r"^[^_]+(_[^_]+)?(_[^_]+)?_[^_]+$", config.TRAIN.MODEL_FILE_NAME):
            ...
            if "Motion" in config.TRAIN.DATA.PREPROCESS.DATA_AUG:
                ...
                model_file_name_parts[train_name_idx] = (
                    "MA-" + model_file_name_parts[train_name_idx]
                )
            ...
            if "Motion" in config.VALID.DATA.PREPROCESS.DATA_AUG:
                ...
                model_file_name_parts[valid_name_idx] = (
                    "MA-" + model_file_name_parts[valid_name_idx]
                )
                config.TRAIN.MODEL_FILE_NAME = "_".join(model_file_name_parts)
            if "Motion" in config.TEST.DATA.PREPROCESS.DATA_AUG:
                model_file_name_parts = config.TRAIN.MODEL_FILE_NAME.split("_")
                model_file_name_parts[test_name_idx] = (
                    "MA-" + model_file_name_parts[test_name_idx]
                )
                config.TRAIN.MODEL_FILE_NAME = "_".join(model_file_name_parts)
        else:
            raise ValueError(
                f"MODEL_FILE_NAME does not follow expected naming pattern of [TRAIN_SET]_[VALID_SET]_[TEST_SET]! \
                             \nReceived {config.TRAIN.MODEL_FILE_NAME}."
            )

    # ENSURE USE_PSEUDO_LABELS IS NOT TRUE FOR UNSUPERVISED METHODS
    if (
        config.TOOLBOX_MODE == "unsupervised_method"
        and config.UNSUPERVISED.DATA.PREPROCESS.USE_PSUEDO_PPG_LABEL == True
    ):
        raise ValueError(
    ...
    if config.UNSUPERVISED.DATA.EXP_DATA_NAME == "":
        config.UNSUPERVISED.DATA.EXP_DATA_NAME = "_".join(
            [
                ...
                "ClipLength{0}".format(
                    str(config.UNSUPERVISED.DATA.PREPROCESS.CHUNK_LENGTH)
                ),
                "DataType{0}".format(
                    "_".join(config.UNSUPERVISED.DATA.PREPROCESS.DATA_TYPE)
                ),
                "DataAug{0}".format(
    ...
    if not ext:  # no file extension
        FOLD_STR = (
            "_" + config.UNSUPERVISED.DATA.FOLD.FOLD_NAME
            if config.UNSUPERVISED.DATA.FOLD.FOLD_NAME
            else ""
        )
        config.UNSUPERVISED.DATA.FILE_LIST_PATH = os.path.join(
            ...
            + "_"
            + str(config.UNSUPERVISED.DATA.BEGIN)
            + "_"
    ...
    if ext == ".csv" and config.UNSUPERVISED.DATA.DO_PREPROCESS:
        raise ValueError(
            "User specified UNSUPERVISED dataset FILE_LIST_PATH .csv file already exists. \
